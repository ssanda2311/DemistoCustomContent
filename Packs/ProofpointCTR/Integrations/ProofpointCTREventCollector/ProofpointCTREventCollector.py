...
class Client(BaseClient):
    ...
    def __init__(self, base_url, client_id, client_secret, headers, verify=False, proxy=False):
        ...
        self.oauth = AuthClient('https://auth.proofpoint.com', client_id, client_secret, verify, proxy)
    ...
    def proofpoint_get_incidents(self, payload):
        updated_headers = {**self.headers, "Authorization": self.get_auth_token()}
        response = self._http_request(
            'POST',
            '/api/v1/tric/incidents',
...
def reduce_timestamp_factor(timestamp_str):
    ...
    format the ISO datetime string to custom format as %Y-%m-%d %H:%M:%S.%f
    """
    ...
    return new_dt.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
...
def get_last_run(data: list[dict]) -> dict:
    ...
    """
    latest_ts = data[-1]["createdAt"]
    incident_ids = [data_item["id"] for data_item in data if data_item["createdAt"] == latest_ts]
...
def deduplicate_incidents(data: list[dict], incidents_ids: list[str], fetch_time: str) -> list[dict]:
    ...
    updated_data = [data_item for data_item in data if
                        ...
                        convert_timestamp_str_to_dt(data_item["createdAt"]) >= convert_timestamp_str_to_dt(fetch_time)
                    ]

    return updated_data


def overlap_exists(data: list[dict], incidents_ids: list[str]) -> bool:
    """
    Filter to check if incident ids from previous fetch are present in new fetch or not
    """
    current_incident_ids = [data_item["id"] for data_item in data]
    return any(incident_id in current_incident_ids for incident_id in incidents_ids)
...
def fetch_events(client: Client, fetch_limit: int, first_fetch: str, last_run: dict) -> list[dict]:
    ...
    """
    last_fetch = last_run.get('created_at')
    ...
    if not last_fetch:
        ...
        last_fetch = first_fetch_dt.isoformat(timespec='milliseconds')

    payload = {
        "filters": {
            "time_range_filter": {
                "start": format_timestamp_str(last_fetch), 
                "end": current_utc_isoformat()
            }
        },
        "endRow": fetch_limit,
        "sortParams": [{
    ...
    incidents = client.proofpoint_get_incidents(payload)

    '''
    Check if refetch is required based on following parameters:
    - If incidents are fetched in current run
    - If incident ids are present from last fetch
    - If no overlapping incidents are found
    '''
    if incidents and last_incident_ids and not overlap_exists(incidents, last_incident_ids):
        # refetch logic
        refetch_time = reduce_timestamp_factor(last_fetch)
        payload["filters"]["time_range_filter"]["start"] = refetch_time

        demisto.updateModuleHealth(f"No overlapping id found in incidents. Refetching with grace time(seconds): {refetch_time}")
        demisto.debug(f"No overlapping id found in incidents. Refetching with grace time(seconds): {refetch_time}")
        incidents = client.proofpoint_get_incidents(payload)

        if not overlap_exists(incidents, last_incident_ids):
            demisto.updateModuleHealth("Still no overlapping id found in incidents after refetch. Possible API data loss.")
            demisto.debug("Still no overlapping id found in incidents after refetch. Possible API data loss.")
...
def test_module(client: Client):
    ...
    incidents = client.test_module(payload)
    if incidents:
        return_results('ok')
...
def main() -> None:
    ...
    try:
        fetch_limit = params.get("eventFetchLimit", 20)
